...
type EditableFields = Partial<Pick<BackendReviewItem, "title" | "movie_title" | "rank" | "content">>;
...
export default function EditReviewDialog({ open, onOpenChange, post, onSaved }: Props) {
  ...
  const buildPayload = (): EditableFields => {
    ...
    if (editMovieTitle.trim() !== initial.movie_title) payload.movie_title = editMovieTitle.trim();
    if (editRank !== initial.rank) payload.rank = editRank;
    if (editContent.trim() !== initial.content) payload.content = editContent.trim();
