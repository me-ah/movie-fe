...
import { getAdminUserList, type AdminUserItem } from "@/api/admin";
...
export default function MembersPanel() {
  ...
  const members: Member[] = useMemo(() => {
    ...
    return sliced.map((u, idx) => {
      ...
      // ✅ role/active 필드가 API에 없어서 "임시"로 계산
      // (나중에 서버가 is_staff/is_active 같은 필드를 주면 거기에 맞춰 수정)
      const role: Member["role"] =
        String(u.username).toLowerCase().includes("admin") ? "관리자" : "일반";
      const active = true;
              ...
              {loading ? (
                ...
                members.map((m) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-zinc-200">{m.name}</td>
