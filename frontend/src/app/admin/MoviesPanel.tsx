...
import { getAdminMovieList, type AdminMovieItem } from "@/api/admin";
...
export default function MoviesPanel() {
  ...
  const formatGenre = (m: AdminMovieItem) => {
    ...
    if (Array.isArray(m.genres) && m.genres.length > 0) return String(m.genres.join(", "));
    return "-";
  };
  ...
  return (
    <>
      <header className="mb-5">
        ...
        <p className="mt-2 text-sm text-zinc-400">영화 데이터를 조회/관리합니다.</p>
      </header>
        ...
        <div className="border-b border-zinc-800 px-5 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button className="bg-blue-500 hover:bg-blue-600">영화 추가</Button>
            </div>
          </div>
        </div>
          ...
          <table className="w-full min-w-[900px]">
            <thead>
              <tr className="bg-zinc-950/40">
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">번호</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">영화명</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">개봉일</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">평점</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">인기도</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400">장르</th>
              </tr>
            </thead>
            ...
            <tbody>
              {loading ? (
                ...
                movies.map((m, idx) => (
                  <tr key={String(m.id)} className="border-t border-zinc-800/80 hover:bg-zinc-950/30">
                    <td className="px-6 py-4 text-sm text-zinc-200">{(page - 1) * movies.length + idx + 1}</td>
                    <td className="px-6 py-4 text-sm text-zinc-200">
                      <div className="max-w-[260px] truncate">{titleOf(m)}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-zinc-300">{formatDate(m.release_date)}</td>
                    <td className="px-6 py-4 text-sm text-zinc-200">{voteAvg(m)}</td>
                    <td className="px-6 py-4 text-sm text-zinc-300">{popularity(m)}</td>
                    <td className="px-6 py-4 text-sm text-zinc-300">{formatGenre(m)}</td>
                  </tr>
                ))
              )}
            </tbody>
